...
export default async (req, res) => {
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      const selectSql = 'SELECT image_patch FROM dsn WHERE dsn_id = ?';
      const [result] = await db.promise().query(selectSql, [dsn_id]);

      if (result.length > 0) {
        const filePatch = result[0].image_patch;

        // Delete file from the system if filePatch exists
        if (filePatch) {
          const filePath = path.join(process.cwd(), 'public', 'uploads','image', filePatch);
          fs.unlinkSync(filePath);
        }

        // Update file_patch to NULL in the database
        const updateSql = 'UPDATE dsn SET image_patch = NULL WHERE dsn_id = ?';
