            ...
            <div className="py-1 md:py-2 px-4 max-w-screen-xl mx-auto flex flex-row flex-wrap justify-between items-center md:max-w-3xl xl:max-w-7xl">
                <div>
                    <Link href="https://lp2m.uinsgd.ac.id/" target="_blank" className="flex items-center py-2 space-x-2 md:space-x-4">
                        ...
                        <span>
                        <p className="md:leading-5 text-[7px] md:text-[12px] font-semibold text-white">Lembaga Penelitian dan Pengabdian Kepada Masyarakat</p>
                        <p className="md:leading-3 text-[10px] md:text-base font-extrabold text-white">UIN SUNAN GUNUNG DJATI BANDUNG</p>
                        </span>
                    </Link>
                </div>
                <div className="w-max flex items-center md:space-x-10 space-x-2 font-bold">
                    ...
                    <Link href={"/login"} className="text-black bg-white md:bg-GreenHerb py-2 md:py-3 px-4 md:px-8 rounded-lg hover:bg-[#8FBB63] text-[10px] md:text-base transition ease-in-out duration-200">Login</Link>
                    <button className="md:hidden p-2 z-50 cursor-pointer" onClick={handleMobileNav}>
                        <Bars3Icon className="w-6 h-6"/>
                    </button>
                    <div className={mobileNav ? 'fixed bg-GreenHerb/80 shadow-md right-0 top-0 w-60 h-screen px-4 py-20 ease-in duration-300 z-0' : 'absolute top-0 h-screen px-4 py-20 -right-96 ease-in duration-300 z-0'}>
